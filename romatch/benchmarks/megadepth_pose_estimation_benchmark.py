...
class MegaDepthPoseEstimationBenchmark:
    def __init__(self, data_root="data/megadepth", scene_names=None) -> None:
        ...
        self.scenes = [np.load(f"{data_root}/{scene}", allow_pickle=True) for scene in self.scene_names]
        ...
        with torch.no_grad():
            ...
            for scene_ind in range(len(self.scenes)):
                ...
                for pairind in tqdm(pair_inds):
                    ...
                    sparse_matches, _ = model.sample(dense_matches, dense_certainty, 5_000)
                    ...
                    if (
                        True
                    ):  # Note: we keep this true as it was used in DKM/RoMa papers. There is very little difference compared to setting to False.
