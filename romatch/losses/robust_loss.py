    ...
    def forward(self, corresps, batch):
        ...
        for scale in scales:
            ...
            if scale_gm_cls is not None:
                ...
                gm_loss = (
                    self.ce_weight * gm_cls_losses[f"gm_certainty_loss_{scale}"] + gm_cls_losses[f"gm_cls_loss_{scale}"]
                )
            ...
            if delta_cls is not None:
                delta_cls_losses = self.delta_cls_loss(
                    x2, prob, flow_pre_delta, delta_cls, scale_certainty, scale, offset_scale
                )
                delta_cls_loss = (
                ...
                reg_loss = (
                    self.ce_weight * delta_regression_losses[f"delta_certainty_loss_{scale}"]
