...
class MegadepthScene:
    def __init__(
        ...
        if len(self.pairs) > max_num_pairs:
            pairinds = np.random.choice(np.arange(0, len(self.pairs)), max_num_pairs, replace=False)
    ...
    def horizontal_flip(self, im_A, im_B, depth_A, depth_B, K_A, K_B):
        ...
        flip_mat = torch.tensor([[-1, 0, self.wt], [0, 1, 0], [0, 0, 1.0]]).to(K_A.device)
        K_A = flip_mat @ K_A
        K_B = flip_mat @ K_B
    ...
    def rand_shake(self, *things):
        ...
        return [tvf.affine(thing, angle=0.0, translate=list(t), scale=1.0, shear=[0.0, 0.0]) for thing in things], t
...
class MegadepthBuilder:
    def __init__(self, data_root="data/megadepth", loftr_ignore=True, imc21_ignore=True) -> None:
        ...
        self.imc21_scenes = set(
            ["0008.npy", "0019.npy", "0021.npy", "0024.npy", "0025.npy", "0032.npy", "0063.npy", "1589.npy"]
        )
