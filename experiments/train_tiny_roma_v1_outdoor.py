...
from romatch.benchmarks import (
    HpatchesHomogBenchmark,
    Mega1500PoseLibBenchmark,
    MegadepthDenseBenchmark,
    MegaDepthPoseEstimationBenchmark,
    ScanNetBenchmark,
    ScanNetPoselibBenchmark,
)
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, bias=False, relu=True
    ):
        super().__init__()
...
class XFeatModel(nn.Module):
    ...
    def __init__(
        self, xfeat=None, freeze_xfeat=True, sample_mode="threshold_balanced", symmetric=False, exact_softmax=False
    ):
        super().__init__()
        if xfeat is None:
    ...
    def pos_embed(self, corr_volume: torch.Tensor):
        ...
        if not self.training and not self.exact_softmax:
            ...
            P_lowres = torch.cat(
                (cv[:, ::down, ::down].reshape(B, H1 * W1 // down**2, H0, W0), best_match[:, None]), dim=1
            ).softmax(dim=1)
    ...
    def visualize_warp(
        ...
        if not isinstance(im_A, torch.Tensor):
            ...
            if symmetric:
                x_A = im_A
            x_B = im_B
        im_A_transfer_rgb = F.grid_sample(x_B[None], warp[:, :W, 2:][None], mode="bilinear", align_corners=False)[0]
        if symmetric:
            im_B_transfer_rgb = F.grid_sample(x_A[None], warp[:, W:, :2][None], mode="bilinear", align_corners=False)[0]
    ...
    def match(self, im0, im1, *args, batched=True):
        ...
        # return 1,1
        flow = (
            F.interpolate(corresps[4]["flow"], size=(H0, W0), mode="bilinear", align_corners=False)
            .permute(0, 2, 3, 1)
            .reshape(B, H0, W0, 2)
        )
        grid = (
    ...
    def sample(
        ...
        good_samples = torch.multinomial(
            certainty, num_samples=min(expansion_factor * num, len(certainty)), replacement=False
        )
        ...
        balanced_samples = torch.multinomial(p, num_samples=min(num, len(good_certainty)), replacement=False)
    ...
    def forward(self, batch):
        ...
        feats_x1_c_warped = F.grid_sample(
            feats_x1_c, coarse_matches.permute(0, 2, 3, 1)[..., :2], mode="bilinear", align_corners=False
        )
        ...
        coarse_matches_up = F.interpolate(
            coarse_matches, size=feats_x0_f.shape[-2:], mode="bilinear", align_corners=False
        )
        coarse_matches_up_detach = coarse_matches_up.detach()  # note the detach
        feats_x1_f_warped = F.grid_sample(
            feats_x1_f, coarse_matches_up_detach.permute(0, 2, 3, 1)[..., :2], mode="bilinear", align_corners=False
        )
        fine_matches_delta = self.fine_matcher(
            torch.cat((feats_x0_f, feats_x1_f_warped, coarse_matches_up_detach[:, :2]), dim=1)
        )
...
def train(args):
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[(9 * N / romatch.STEP_SIZE) // 10])
    ...
    for n in range(romatch.GLOBAL_STEP, N, k * romatch.STEP_SIZE):
        mega_sampler = torch.utils.data.WeightedRandomSampler(mega_ws, num_samples=batch_size * k, replacement=False)
...
def test_mega_8_scenes_poselib(model, name):
    mega1500_benchmark = Mega1500PoseLibBenchmark(
        "data/megadepth",
        num_ransac_iter=1,
        test_every=1,
        scene_names=[
